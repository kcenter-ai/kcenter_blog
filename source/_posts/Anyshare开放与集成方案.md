...
#### 6.1.1 搜索准确率
- **关键字检索**：在百万级文档中，Top-10 准确率 > 85%（基于 BM25 算法）。
- **向量检索**：在相同数据集下，Top-10 相似度准确率 > 90%（基于 BGE 模型）。
- **混合检索（RFF）**：结合两者优势，Top-10 准确率 > 92%。

#### 6.1.2 召回能力
- **多源召回**：可同时从文档中心、WikiDoc、FAQ 知识库中召回相关内容。
- **上下文补全**：对召回切片自动补充前后文，提升答案完整性。
- **权限过滤**：严格遵循文档访问权限，仅返回用户有权访问的内容。

#### 6.1.3 处理能力
- **文档解析**：支持 50+ 种文件格式（Office、PDF、TXT、代码文件等）。
- **模型服务**：文本向量化、重排序、OCR、语音转文字等 AI 能力开箱即用。

### 6.2 性能指标

#### 6.2.1 接口响应时间（P95，单位：毫秒）
| 接口类别 | 简单请求 | 复杂请求 |
|----------|----------|----------|
| 元数据查询 | < 50ms | < 100ms |
| 内容解析 | < 200ms | < 500ms |
...
- **并发连接**：支持 1000+ 并发请求。
- **吞吐量**：峰值 QPS > 500。
- **数据规模**：支持亿级文档切片索引，TB 级向量数据。

#### 6.2.3 可用性与可靠性
- **服务可用性**：99.9%（按月统计）。
- **数据一致性**：最终一致性，索引延迟 < 30 秒。
- **故障恢复**：MTTR < 5 分钟。

### 6.3 监控与日志

#### 6.3.1 监控指标
- **业务指标**：请求量、成功率、响应时间分位值（P50/P95/P99）。
- **系统指标**：CPU/内存使用率、磁盘 IO、网络流量。
- **质量指标**：检索准确率、召回率、用户满意度。

#### 6.3.2 日志规范
- **访问日志**：记录所有请求的端点、状态码、耗时、调用方。
- **审计日志**：记录敏感操作（文档访问、权限变更）。
- **错误日志**：记录异常堆栈、上下文信息，便于排查问题。
...
**Q1：应用账号可以访问哪些文档？**
A：应用账号的权限由管理员配置，可指定具体的 GNS 路径范围。默认无权限，需显式授权。

**Q2：访问令牌过期后如何处理？**
A：实现令牌自动刷新机制，在收到 401 错误时重新获取令牌并重试请求。

**Q3：如何控制用户级访问权限？**
A：使用用户令牌（而非应用账号）调用接口，系统会自动应用该用户的文档 ACL。

### 7.2 接口使用

**Q4：`scroll_id` 的有效期是多久？**
A：每次调用都会刷新过期时间，但最长不超过 1 分钟。建议在获取 `scroll_id` 后尽快使用。

**Q5：如何优化大量文档的遍历性能？**
A：使用 `scroll` 分页，并合理设置 `limit` 参数（建议 100-500）。避免单次请求返回过多数据。

**Q6：向量检索时 embedding 维度是多少？**
A：默认使用 1024 维向量（BGE 模型），其他模型可能不同。可通过接口查询具体维度。

### 7.3 数据处理

**Q7：文档更新后，索引多久生效？**
A：索引异步更新，通常在文档修改后 30 秒内生效。

**Q8：支持哪些文件格式的全文解析？**
A：支持常见格式：PDF、DOC/DOCX、PPT/PPTX、XLS/XLSX、TXT、HTML、Markdown 等。

**Q9：音视频文件转写的准确率如何？**
A：中文普通话转写准确率 > 95%，英文 > 97%，支持背景噪音抑制。

### 7.4 故障排查

**Q10：收到 403 错误怎么办？**
A：检查应用账号是否有该文档的访问权限，或使用用户令牌重试。

**Q11：接口响应超时可能的原因？**
A：网络问题、服务器负载过高、请求参数过于复杂（如过大 embedding）。可适当调整 `timeout` 参数。

**Q12：如何获取技术支持？**
A：联系 Anyshare 管理员或通过工单系统提交问题，提供请求 ID、错误信息、复现步骤。
...
| 400000000 | 400 | 参数错误 |
| 401000000 | 401 | 未授权 |
| 403000000 | 403 | 无法执行此操作 |
| 404000000 | 404 | 资源不存在 |
| 409000000 | 409 | 资源冲突 |
| 500000000 | 500 | 内部错误 |

完整错误码列表请参考 OpenAPI 文档中的 `Error` 模型定义。
...
> **提示**：本文档持续更新，最新版本请访问 Anyshare 官方文档站点。