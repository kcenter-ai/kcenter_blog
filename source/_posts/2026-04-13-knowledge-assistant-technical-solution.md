---
title: 知识助手技术方案 - 企业智能知识管理的多Agent协作引擎
date: 2026-04-13
categories:
  - 技术方案
tags:
  ...
  - 智能问答
  - 文档写作
description: 基于多Agent协作的企业知识助手系统技术方案，实现智能化文档写作、精准搜索、规则审阅与问答服务的一体化解决方案
---
...
#### STEP 1: 获取当前时间
- **目的**: 获取系统当前时间，为后续相对时间转换提供基准
- **实现**: 获取UTC时间 → 转换为北京时间(UTC+8) → 格式化输出
- **输出**: `current_time_str` 供后续步骤使用

#### STEP 2: 预处理（信息提取）
- **Agent类型**: LLM Expert模式
- **任务**: 从用户Query中提取关键信息
- **提取字段**:
  - `username`: 用户名（如"user123"、"@admin"，或间接提及如"通知张三"）
  - `time`: 时间信息（支持绝对时间和相对时间，如"明天"、"上周"）
- **时间转换规则**: 将相对时间转换为绝对时间格式 `2007-01-01-00:00:00`
- **示例输出**:
  ```json
  {"username": "user123", "time": "2025-08-21-15:00:00"}
  ```

#### STEP 3: 文档召回（RecallPipeline）
召回Pipeline是整个文档问答的核心，包含以下子步骤：

| 子步骤 | 功能说明 | 关键技术 |
|--------|----------|----------|
| **Step 3.1 多路并发召回** | 同时查询知识图谱、文档库、FAQ库三类数据源 | Embedding向量检索 + 关键词匹配 |
| **Step 3.2 MD5去重** | 对召回切片进行MD5去重，保留最高分数副本 | 哈希去重算法 |
| **Step 3.3 Rerank重排序** | 使用Rerank模型对候选切片进行语义相关性重排序 | Cross-Encoder语义模型 |
| **Step 3.4 对象化与聚合** | 将切片按文档ID聚合成文档对象，取Top N（默认8个） | 分组聚合算法 |
| **Step 3.5 上下文扩充** | 对命中切片进行前后文扩充（默认前后各1个切片） | Slice Fetch API |

**召回配置参数**:
```json
{
  "MAX_DOC_NUM": 8,              // 最终保留文档数
  "BEFORE_SLICE_NUM": 1,         // 向前扩充切片数
  "AFTER_SLICE_NUM": 1,          // 向后扩充切片数
  "RECALL_MIN_SCORE": 0.5,       // 初排召回阈值
  "RERANK_MIN_SCORE": 0.2,       // Rerank后最低分数
  "TOP_N_RECALL_MAP": {
    "doc": 60,                   // 普通文档召回数
    "wikidoc": 30,               // Wiki文档召回数
    "faq": 10                    // FAQ召回数
  },
  "MAX_SLICES_FOR_RERANK": 300   // Rerank最大切片数
}
```

#### STEP 4: 移除重复文件并排序
综合处理召回结果，输出高质量候选文档集合：

1. **按文件名去重**: 同一文件多次命中只保留第一次出现的副本
2. **按用户名过滤**: 如Query中指定"张三的文档"，只保留该用户创建的文档
3. **按时间过滤**: 如Query中指定"2024年的文件"，只匹配对应年份的文档
4. **时效过滤**: 自动过滤2年前的过期文档，确保信息时效性
5. **版本智能排序**: 按版本号排序（数字版本: 7.0 > 6.0；字母版本: v3 > v2）

**特殊处理**: FAQ项和用户上传内容(USER_INPUT)直接保留，不参与排序筛选

#### STEP 5: 整理召回数据结构
将召回结果转换为LLM友好的字典格式：

**输入**: `[{meta, content, retrieve_source_type}, ...]`
**输出**: `{"文件名1": ["段落1", "段落2"], "文件名2": ["段落1"], ...}`

**处理逻辑**:
- DOC_LIB: 使用 `meta.doc_name` 作为键
- FAQ: 使用 `meta.title[0]` 作为键
- USER_INPUT: 使用 `meta.doc_name = "参考文档N.txt"` 作为键

#### STEP 6: LLM生成答案
基于召回的参考文档生成最终答案：

**System Prompt要点**:
- 严格锚定提供的文档内容，不加入文档外知识
- 保留参考文档中的图片地址
- 禁止生成假的图片链接格式

**答案生成核心要求**:
1. **完整性**: 逐句检查所有相关文档，确保无遗漏
2. **准确性**: 严格依据文档，禁止推测或编造
3. **精简性**: 避免重复，不展开无关背景知识
4. **冲突处理**: 多文档冲突时，优先新版本，标注旧版本差异
5. **图片保留**: 引用段落时保留其中的图片地址
6. **LaTeX公式**: 使用单反斜杠原始格式，禁止转义

**无信息处理**: 如未召回相关内容，返回"很抱歉，目前在云盘文件中未查询到与[问题]相关的信息..."
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| `as_ctx` | object | 是 | 上下文对象，包含用户信息、权限、工具参数等 |
